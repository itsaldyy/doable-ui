...
describe('useTodos', () => {
  ...
  describe('Error Handling on Load', () => {
    it('should handle StorageUnavailableError', async () => {
      ...
      expect(result.current.error).toContain('localStorage is not available');
      expect(result.current.todos).toEqual([]);
    });
  ...
  describe('addTodo', () => {
    it('should add a new todo', async () => {
      ...
      expect(mockedStorage.saveTodos).toHaveBeenCalledWith(result.current.todos);
    });
  ...
  describe('updateTodo', () => {
    ...
    it('should update todo text', async () => {
      ...
      act(() => {
        result.current.updateTodo('test-id-1', { text: 'Updated text' });
      });

      const updatedTodo = result.current.todos.find((t) => t.id === 'test-id-1');
    ...
    it('should trim whitespace when updating text', async () => {
      ...
      act(() => {
        result.current.updateTodo('test-id-1', { text: '  Updated with spaces  ' });
      });
    ...
    it('should handle non-existent todo ID', async () => {
      ...
      act(() => {
        result.current.updateTodo('non-existent-id', { text: 'Updated' });
      });
